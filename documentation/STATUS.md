...
- SQLite-backed throttling for login, forgot-password, and reset-password attempts, plus audit events for sensitive admin/account mutations.
...
- Import accepts safe URLs from JSON arrays, object-wrapped exports, browser/Netscape HTML, and newline URL lists; imported bookmarks keep their detected source; queued import processing updates fetched, AI-processed, failed, completed status counters, aggregate source reports, and bounded item provenance on import detail. Arivu full JSON backups restore bookmark details, summaries, tags and aliases, annotations, linked and standalone notes, saved searches, review events, and provenance under the importing user. CSV, browser HTML, and Markdown remain lightweight bookmark interchange formats, while Obsidian ZIP export creates vault-ready bookmark and note folders from the same user-scoped data.
...
- Extension selected-text saves create quote annotations through the extension audience route, popup saves can include quick notes and manual tags, self-hosted origins can be granted through popup settings instead of manifest edits, and the popup uses native system fonts without remote font requests.
- The embedded UI now exposes graph, duplicates, daily memory, review queue actions for bookmarks and standalone notes, summaries, related items, tag/domain/source/read-status/date-range retrieval filters, profile and password settings, admin API key settings, admin audit log visibility, X connection controls, tags, tag alias management, editable annotations, standalone and linked notes, saved searches, cited answer mode synthesized from saved summaries/snippets/standalone notes with citations, read state, review completion, analytics signals, and visible processing status.
...
Current coverage includes schema initialization, sanitizer allowlist behavior, safe URL validation, auth endpoint throttling, audit-event writes and admin audit visibility for sensitive mutations, provider setting allowlisting, Go module checksum verification, govulncheck scanning, embedded frontend and extension JavaScript syntax checks, extension URL/origin self-tests, import URL extraction, import source detection, imported bookmark source persistence, import job payload/progress/source-report/provenance accounting, full JSON backup export and restore coverage, Obsidian ZIP export coverage, Markdown export escaping, migration unknown-field rejection, direct HTTP handler integration tests, collection membership ownership checks, second-brain route scoping and CSRF checks, annotation update/delete routes, tag/date/source filtering, cited answer mode with synthesized bookmark summary and standalone note context, bookmark and standalone note review queues, browser-facing first-run and PWA manifest contracts, and golden parity fixtures.
...
- Cited answer mode synthesized from saved context.
