...
func FromEnv() Config {
	return Config{
		...
		BrowserCapture: BrowserCaptureConfig{
			...
			Screenshot:        envBool("ARIVU_BROWSER_CAPTURE_SCREENSHOT", false),
			...
			Timeout:           envDuration("ARIVU_BROWSER_CAPTURE_TIMEOUT", 30*time.Second),
			MaxFileBytes:      envInt64("ARIVU_BROWSER_CAPTURE_MAX_FILE_BYTES", 10<<20),
			MaxTotalBytes:     envInt64("ARIVU_BROWSER_CAPTURE_MAX_TOTAL_BYTES", 20<<20),
		},
	}
}

func (c Config) Validate() error {
	if c.BrowserCapture.Enabled && !c.BrowserCapture.Screenshot && !c.BrowserCapture.PDF && !c.BrowserCapture.SelfContainedHTML {
