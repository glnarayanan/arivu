...
func (s *Service) LibraryItems(w http.ResponseWriter, r *http.Request, user auth.User) {
	...
	for rows.Next() {
		var rowUserID, id, itemType, title, body, source, stage, topic, connection, created, updated, thumbnail string
		if err := rows.Scan(&rowUserID, &id, &itemType, &title, &body, &source, &stage, &topic, &connection, &created, &updated, &thumbnail); err != nil {
			writeError(w, http.StatusInternalServerError, "Could not load library")
			return
		}
		title = knowledgeDisplayTitle(itemType, title, body)
		item := map[string]any{"id": id, "type": itemType, "title": title, "body": body, "source": source, "stage": stage, "topic": topic, "connection": connection, "created_at": created, "updated_at": updated, "thumbnail": thumbnail}
...
var libraryUnion = `SELECT * FROM (
 SELECT b.user_id,b.id,'bookmark' item_type,COALESCE(b.title,b.url) title,CASE WHEN lower(trim(COALESCE(b.description,'')))=lower(trim(COALESCE(b.title,b.url))) THEN substr(COALESCE(NULLIF(trim(b.text_content),''),b.description,''),1,600) ELSE substr(COALESCE(NULLIF(trim(b.description),''),b.text_content,''),1,600) END body,b.source,COALESCE(st.stage,'inbox') stage,
  COALESCE((SELECT group_concat(value,' ') FROM (SELECT entity value FROM bookmark_entities topic_entity WHERE user_id=b.user_id AND bookmark_id=b.id AND ` + semanticEligibilitySQL("topic_entity") + ` UNION SELECT concept FROM bookmark_concepts topic_concept WHERE user_id=b.user_id AND bookmark_id=b.id AND ` + semanticEligibilitySQL("topic_concept") + `)),'') topic,
	  CASE WHEN EXISTS(SELECT 1 FROM item_links l WHERE l.user_id=b.user_id AND ((l.from_type='bookmark' AND l.from_id=b.id) OR (l.to_type='bookmark' AND l.to_id=b.id))) THEN 'connected' ELSE 'unconnected' END connection_state,b.created_at,b.updated_at,CASE WHEN b.thumbnail LIKE '/api/media/%' THEN b.thumbnail ELSE '' END thumbnail
